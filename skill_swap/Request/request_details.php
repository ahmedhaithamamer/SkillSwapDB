            ...
            <div class="request-container">
                ...
                <div class="card shadow-sm mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <span class="badge bg-<?php echo $is_owner ? 'success' : 'primary'; ?>">
                            <?php echo $is_owner ? 'Your Request' : 'Other\'s Request'; ?>
                        </span>
                    </div>
                    <div class="card-body">
                ...
                <div class="card shadow-sm mb-4">
                    <div class="card-header">
                        <h4 class="mb-0">Responses (<?php echo count($responses); ?>)</h4>
                    </div>
                    <div class="card-body">
                        <?php if (empty($responses)): ?>
                            <p class="text-center text-muted my-4">No responses yet</p>
                        <?php else: ?>
                            <?php foreach ($responses as $response): ?>
                                <div class="response-card p-3 <?php echo ($response['user_id'] == $request['user_id']) ? 'owner' : ''; ?>">
                                    <div class="d-flex justify-content-between">
                                        <div>
                                            <h6><?php echo htmlspecialchars($response['responder_name']); ?></h6>
                                            <div class="response-meta">
                                                <?php echo date('F j, Y, g:i a', strtotime($response['created_at'])); ?>
                                                <span class="response-type bg-<?php echo ($response['response_type'] == 'money') ? 'success' : (($response['response_type'] == 'skill') ? 'primary' : 'warning'); ?>">
                                                    <?php 
                                                    if ($response['response_type'] == 'money') {
                                                        echo '<span class="bi bi-cash text-white"> <b>Money</b></span> ';
                                                    } elseif ($response['response_type'] == 'skill') {
                                                        echo '<span class="bi bi-tools text-white"> <b>Skill</b></span> ';
                                                    } else {
                                                        echo '<span class="bi bi-shuffle text-white"> <b>Both</b></span> ';
                                                    }
                                                    ?>
                                                </span>
                                                <?php if ($is_owner): ?>
                                                    <a href="./accept_response.php?response_id=<?php echo $response['id']; ?>" class="btn btn-sm btn-success accept-response" data-response-id="<?php echo $response['id']; ?>">
                                                        Accept
                                                    </a>
                                                <?php endif; ?>
                                            </div>
                                    ...
                                    <p class="mt-2 mb-0"><?php echo nl2br(htmlspecialchars($response['note'])); ?></p>
                                </div>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </div>
                ...
                <!-- Add Response Form (only show if not the request owner and hasn't responded yet) -->
                <?php if (!$is_owner && !$has_responded): ?>
                <div class="card shadow-sm">
                    <div class="card-header">
                        <h4 class="mb-0">Add Your Response</h4>
                    </div>
                    <div class="card-body">
                        <form method="POST" action="">
                            <div class="mb-3">
                                <label for="responseType" class="form-label">What are you interested in?</label>
                                <select class="form-select" id="responseType" name="response_type" required>
                                    <option value="" disabled selected>Select your interest</option>
                                    <?php if ($request['offer_money'] > 0): ?>
                                        <option value="money">Money (<?php echo $request['offer_money']; ?> EGP)</option>
                                    <?php endif; ?>
                                    <?php if (!empty($offered_skills)): ?>
                                        <option value="skill">Skill Exchange</option>
                                    <?php endif; ?>
                                    <?php if ($request['offer_money'] > 0 && !empty($offered_skills)): ?>
                                        <option value="both">Both Money and Skill</option>
                                    <?php endif; ?>
                                </select>
                            </div>
                            <div class="mb-3">
                                <label for="responseMessage" class="form-label">Your Message</label>
                                <textarea class="form-control" id="responseMessage" name="response_message" rows="4" placeholder="Introduce yourself and explain how you can help..." required></textarea>
                            </div>
